...
<!--             <form action="/newQ&A" method="get" id=frmNew> -->
			...
			<tr>
			<input type=hidden name=num value="${one}" readonly class="border border-success p-2 mb-2">
			<input type=hidden id=type value="${token}" readonly class="border border-success p-2 mb-2">
			</td>
			<tr><td>내용</td>
...
// })
     ...
     .on('click','#btnInsert',function(){
     	...
     	$.ajax({url:'/revpInsert',type:'post',data:{prod_num:${one}, RCONTENT:$('#Bcontent').val() },dataType:'text',
     		beforeSend:function(){
     		},
     		...
     		if($('#type').val()=="room"){
     			$.ajax({url:'/revInsert',type:'post',data:{room_num:${one}, RCONTENT:$('#Bcontent').val() },dataType:'text',
     	     		beforeSend:function(){
     	     		},
     	     		success:function(data){
